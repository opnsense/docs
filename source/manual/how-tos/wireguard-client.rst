...
.. Tip::
    ...
    Peers can be generated using the new peer generator feature under :menuselection:`VPN --> WireGuard --> Peer generator`.
    If using the peer generator and require Unbound DNS to serve names, fill the DNS server with the tunnel address (eg :code:`10.10.10.1` ).
