...
Services running on the router and configured to use the VPN interface will generate traffic using the VPN address as the source IP. This traffic has to be routed to the VPN gateway. Note that locally generated traffic is not affected by NAT or by the firewall rule created in Step 8.
